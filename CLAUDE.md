...
| 2026-06-17 | 수집 중단 버튼 | popup, background, content, messages(CANCEL) | 수집 중 사용자 중단. chrome.storage 취소 플래그를 background가 set, content 날짜·계좌 루프·runCollect가 확인해 멈춤(저장 안 함, 대기 복귀) |
| 2026-06-17 | 자가호스팅 배포 파이프라인(서명 .crx + force-install 설치기) | package.json, scripts/{keygen,build-crx}.mjs, .github/workflows/release.yml, README, manifest(key) | 개발자 모드 없이 일반 설치 요구 → 웹스토어 밖 .crx는 Chrome이 차단하므로, GitHub Releases에 서명 .crx+updates.xml 호스팅 + HKCU `ExtensionInstallForcelist` 정책 등록 install.bat(관리자 불필요·자동업데이트). manifest `key`로 ID 고정(`bfigkpjffbehjcjdapacmpjimgbdoilb`). v* 태그 push→Actions가 서명·빌드·릴리스 자동화(시크릿 `CRX_PRIVATE_KEY`) |
| 2026-06-17 | 신한투자증권 어댑터 1단계: 정찰(PROBE) | manifest, content/shinhan/{index,page-bridge}, messages(SOURCE.SHINHAN·pin), background(source별 파일선택), popup(신한감지·PIN입력) | 신한 잔고표 렌더방식(table vs RealGrid/AUIGrid 그리드)·거래 PIN 입력칸(일반 vs 보안키보드) 미상 → 실측 진단 PROBE부터. 거래 PIN은 사용자가 수집 시작 시 입력, 메모리만(무저장). background를 다증권사 파일선택(CONTENT_SCRIPTS 맵)으로 일반화. 상세 `_workspace/06_shinhan_recon.md` |
