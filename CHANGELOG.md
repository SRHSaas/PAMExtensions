...
### Added
- **수집 중단 버튼**: 수집(스크랩) 진행 중 팝업에 "수집 중단" 버튼이 나타나 사용자가 멈출 수 있다.
  중단 플래그(`chrome.storage.local`)를 background가 켜고, content script의 날짜·계좌 루프와
  runCollect(타깃 경계)가 매 반복 확인해 현재 항목 직후 멈춘다. 중단 시 저장하지 않고 대기 상태로 복귀.
