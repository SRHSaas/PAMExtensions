...
### 페이지 전역 접근은 MAIN-world 브리지로 (CSP 주의)

증권사 페이지의 SPA 네비게이션·조회는 페이지가 정의한 JS 전역 함수(예: 미래에셋 `openHp`/`subTabChange`)와 데이터 객체(예: `hkd1004.list`)에 의존한다. ISOLATED content script는 이 전역에 접근 못 하고, 증권사 CSP는 인라인 `<script>` 주입과 `eval`을 모두 막는다. → 페이지 전역 접근은 **MAIN-world content script 브리지**로 하되 **eval 없이 고정 명령(call/get 등)**으로 설계한다. DOM 읽기/클릭/테이블 파싱은 브리지로 보내지 말고 동일출처 `iframe.contentDocument`로 ISOLATED에서 직접 한다(브리지는 페이지 JS 전역 접근만). 상세 패턴은 `chrome-extension-mv3` 스킬의 "페이지 월드 접근" 절 참조.
